...
Файл [`streets_chelyabinsk_2024.sql`](./streets_chelyabinsk_2024.sql) содержит таблицу `streets8`:
...
mysql -u USER -p chelyabinsk_streets < streets_chelyabinsk_2024.sql
```

Если база уже создана:

```bash
mysql -u USER -p DATABASE_NAME < streets_chelyabinsk_2024.sql
```
...
Самый простой путь: сначала импортировать `streets_chelyabinsk_2024.sql` в MySQL/MariaDB, а затем выгрузить таблицу в нужный формат.
